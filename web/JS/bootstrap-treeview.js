...
$.fn.addtabs = function (options) {
	...
	Addtabs.options = $.extend({
		...
		close: true, //是否可以关闭
		monitor: '#treeview', //监视的区域
		iframeUse: true, //使用iframe还是ajax
		iframeHeight: 660, //固定TAB中IFRAME高度,根据需要自己修改
		method: 'init',
...
;(function ($, window, document, undefined) {
	...
	*/
	...
	//modify by yang_tfei 20160404 for integrated tabs js
	Tree.prototype.buildTree = function (nodes, level) {
		...
		$.each(nodes, function addNodes(id, node) {
			...
			for (var i = 0; i < (level - 1); i++) {
				...
				treeItem.attr('data-addtab','treeItem_'+ node.nodeId)
					    .attr('data-url',node.url ? node.url: '') ;    //Add url start add by yang_tfei 20160404
			}
